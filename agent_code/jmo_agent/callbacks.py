...
feature_length = ((2 * scan_length) + 1)**2 + 289 + 6

# name of the model to build or load
model_name = "model_rounds_400.pt"
...
def state_to_features(self, game_state: dict) -> np.array:
    ...
    if game_state is None:
        ...
        bombs_times = (5 * np.ones(1)) / 4 # set to 1.25 if no bombs
        x_y_coord = np.zeros(2) # set to False=0 if no bombs, first value for x
        dist_bombs = (5 * np.ones(1)) / 4 # set to 1.25 if no bombs
        ...
        if len(coins_pos) > 0:
            ...
            closest_coin_dist[0] = np.min(coin_dist) / norm_fact

    features = np.concatenate((surrounding, bombs_field, bombs_times, x_y_coord, dist_bombs, crate_density, closest_coin_dist))
